...
def build_report(cases: Sequence[CaseResult], *, generated_at: str) -> ScorecardReport:
    ...
    for suite in SUITES:
        ...
        metrics.append(metric(suite.key, round(rate, 4), threshold=1.0, detail=detail))
    metrics.append(
        metric(
            "policy_drift_count",
            drift,
    ...
    return ScorecardReport.build(generated_at=generated_at, suite=SUITE_SCORECARD, metrics=metrics)
...
def run_pytest_junit(targets: Sequence[str], junit_path: Path, *, cwd: Path = REPO_ROOT) -> None:
