...
def save_pack(store: RunStore, record: SkillPackRecord) -> None:
    ...
    packs[record.pack_id] = record
    store.set_setting(SKILL_PACKS_SETTINGS_KEY, json.dumps([asdict(p) for p in packs.values()]))
...
def run_trial(
    ...
    *,
    ...
    results = [{"script": rel, **_check_script(root / rel)} for rel in record.scripts]
...
def _workspace_grants(record: SkillPackRecord, grants: tuple[str, ...]) -> tuple[str, ...]:
    ...
    for command in grants:
        ...
        rewritten = [
            f"{WORKSPACE_PACK_DIR}/{record.pack_id}/{token}" if token in record.scripts else token
            for token in tokens
        ]
...
def promote_pack(
    ...
    *,
    ...
    if record.state == "suspended":
        ...
        if not evidence["ok"]:
            ...
            failed = (
                ...
                or "; ".join(
                    f"{r['script']}: {r['detail']}" for r in evidence["scripts"] if not r["ok"]
                )
                or "no scripts found"
            )
            raise SkillPackError(
    ...
    template = dataclasses.replace(
        template_from_skill_md(snapshot, allow_scripts=_workspace_grants(record, grants)),
        name=record.pack_id,
...
def suspend_pack(store: RunStore, pack_id: str, *, rollback: bool = False) -> SkillPackRecord:
