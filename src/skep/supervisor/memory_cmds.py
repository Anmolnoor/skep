...
def cmd_memory_reject(args: argparse.Namespace) -> int:
    ...
    try:
        try:
            store.reject_memory_proposal(
                args.proposal_id, actor=CLI_ACTOR, reason=args.reason
            )
        except MemoryError as exc:
            return _err(str(exc))
    finally:
...
def cmd_memory_reject_batch(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        for proposal in targets:
            store.reject_memory_proposal(
                proposal.proposal_id, actor=CLI_ACTOR, reason=args.reason
            )
...
def register_memory_commands(
    ...
    proposals_p.add_argument(
        "--state", default=None, help="filter by state (e.g. pending_review)"
    )
