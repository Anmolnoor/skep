...
PROVIDER_PROTOCOLS: frozenset[str] = frozenset(
    {"ollama", "openai_compat", "anthropic", "gemini"}
)
    ...
    for index, candidate in enumerate(providers):
        ...
        if index == 0:
            return FallbackDecision(
                candidate.provider_id, primary.provider_id, False, tuple(skipped),
                "fallback.primary_healthy",
            )
        return FallbackDecision(
            candidate.provider_id, primary.provider_id, True, tuple(skipped),
