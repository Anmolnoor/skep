...
GROUP_LIST_KEYS = frozenset({"default_network", "allowed_shell_commands", "default_env_allowlist"})
...
def validate_policy_group(name: object, policy: object) -> tuple[str, dict[str, Any]]:
    ...
    if not POLICY_GROUP_NAME_RE.match(group_name):
        raise ValueError(
            f"policy group names are 2-32 chars of [a-z0-9-] starting with a letter, got {name!r}"
        )
...
def delete_policy_group_record(store: RunStore, name: str) -> None:
    ...
    raise ValueError(f"no policy group {name!r}; known: {sorted(stored_policy_groups(store))}")
