...
def _script_run_result(store: RunStore, task_id: str) -> dict[str, Any]:
    ...
    def _capped(text: str) -> str:
        return text if len(text) <= SCRIPT_RUN_OUTPUT_CAP else (
            text[:SCRIPT_RUN_OUTPUT_CAP] + "\n… (truncated; full output in the run artifact)"
        )
...
MUTATING_TOOL_SPECS: list[dict[str, Any]] = [
    ...
    _tool(
        ...
        "swaps to the fork — all in this single confirmation.",
        {
            ...
            "policy": {
                "type": "object",
                "description": "groupable keys only, e.g. "
                '{"default_network": ["api.example.com"]}',
            },
            "fork_from": {
                "type": "string",
    ...
    _tool(
        ...
        "PROPOSE approving a memory proposal into durable memory "
    ...
    _tool(
        "forget_memory",
        "PROPOSE forgetting (soft-deleting) a durable memory item "
        "(requires user confirmation).",
...
_INDEX_BOILERPLATE = re.compile(
    r"^PROPOSE\s+|\s*\(requires[^)]*\)\s*", re.IGNORECASE
)
...
_INDEX_STOPWORDS = frozenset(
    {
        "a", "an", "the", "of", "for", "to", "in", "on", "this", "that", "its",
        "it", "and", "or", "with", "as", "one", "you", "your", "is", "are",
        "be", "from", "by", "at", "into", "when", "what", "new", "up",
    }
)
            ...
            return {
                "schedule": {
                    **_schedule_summary(schedule),
                    ...
                    "last_output": (
                        None
                        if schedule.last_output is None
                        else str(schedule.last_output)[:2000]
                    ),
                    "repo": schedule.repo,
            ...
            "messages": [
                {
                    ...
                    + (" …" if len(record.content) > 500 else ""),
...
def execute_mutation(
    ...
    *,
    ...
    result = _execute_mutation(
        name, args, store=store, holder=holder, runner=runner, actor=actor,
        decision=decision, chat_id=chat_id,
    )
    ...
    *,
    ...
    if name == "promote_tool":
        ...
        if record.state == "rolled_back":
            raise ValueError(
                f"plugin {plugin_id!r} was rolled back — that is terminal; "
                "forge_tool a replacement"
            )
    ...
    if name == "approve_review":
        ...
        warning = actions.reverification_warning(
            store.reverification_for(str(run["task_id"]))
        )
    ...
    if name == "open_pr":
        ...
        if args.get("branch"):
            ...
            if not args.get("repo"):
                raise ValueError(
                    "branch mode needs repo= (registered repo slug or host path)"
                )
    ...
    if name == "setup_project":
        ...
        if args.get("groups"):
            ...
            overrides.setdefault(
                "policy_groups", [str(group) for group in args["groups"]]
            )
    ...
    if name in ("discord_delete_message", "discord_timeout_member"):
        ...
        if channel_config is None or not channel_config.enabled or token is None:
            raise ValueError(
                "the discord channel is not enabled/configured (Settings → channels)"
            )
    ...
    if name == "remember":
        ...
        if memory_class not in MEMORY_CLASSES:
            raise ValueError(
                f"unknown memory class {memory_class!r} — "
                f"pick one of: {', '.join(durable_classes)}"
            )
