...
def skill_index_block(store: RunStore) -> str:
    ...
    return "\n".join(
        [_SKILL_INDEX_HEADER, ", ".join(template.name for template in templates)]
    )
...
def mcp_index_block(store: RunStore) -> str:
    ...
    if not servers:
        return ""
    lines = [
        f"- {server_id} ({config.transport})" for server_id, config in servers.items()
    ]
...
class ChatEngine:
    ...
    def _advertised_tools(
        self, chat: Any, *, read_only: bool = False
    ) -> list[dict[str, Any]]:
    ...
    def _replay(self, chat_id: str, chat: Any, *, budget: int) -> tuple[list[dict[str, Any]], int]:
        ...
        for record in records:
            if record.role == "tool":
                ...
                messages.append(
                    {"role": "tool", "tool_name": record.tool_name, "content": content}
                )
    ...
    def _user_message_with_images(
        self, chat_id: str, record: Any, vision: bool
    ) -> dict[str, Any]:
        ...
        if vision and names:
            images = [
                base64.b64encode(
                    (attachments_dir(self.home, chat_id) / name).read_bytes()
                ).decode("ascii")
                for name in names
            ]
    ...
    def _chat_stream_with_retry(
        ...
        for chunk in chat_stream_with_retry(base_url, api_key, sleep=self.sleep, **kwargs):
            if chunk.get("done") and (
                chunk.get("prompt_eval_count") or chunk.get("eval_count")
            ):
            ...
            for call in tool_calls:
                ...
                if name == CLARIFY_TOOL_NAME:
                    ...
                    if question:
                        ...
                        self.store.add_chat_message(
                            chat_id, role="assistant", content=formatted
                        )
                ...
                if name in MUTATING_TOOL_NAMES:
                    ...
                    action_id = self.store.add_chat_action(
                        ...
                        decided_by=(
                            None
                            if decision is None
                            else decision.decided_by or decision.reason
                        ),
                    )
...
def run_scheduled_prompt(
    ...
    store.add_chat_message(
        chat_id, role="user", content=f"[schedule {schedule.name!r}] {prompt}"
    )
    parts: list[str] = []
    try:
...
def run_analysis_tasks(
    ...
    for task in tasks:
        chat = store.create_chat(
            title=f"analysis: {task[:60]}", model=None, source="analysis"
        )
        try:
    ...
    def _resolve_command(chat_id: str, action_id: str, *, confirm: bool) -> dict[str, Any]:
        ...
        if confirm or gate_mirror:
            ...
            args = (
                action.args
                if confirm
                else {"review_id": str(action.args.get("review_id", ""))}
            )
