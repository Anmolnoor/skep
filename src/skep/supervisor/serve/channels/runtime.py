...
def _telegram_action_keyboard(
    keyboard: dict[str, object], action_id: str
) -> dict[str, object]:
...
def register_slash_commands(
    token: str, application_id: str, register: RegisterCommands
) -> None:
...
class DiscordGateway(threading.Thread):
    ...
    def _handle_message(self, data: dict[str, Any], config: ChannelConfig, token: str) -> None:
        ...
        session_bound = (
            self._store.channel_session(f"{discord_adapter.CHANNEL}:{raw_channel_id}")
            is not None
        )
    ...
    def _resolve_verdict(
        self, action_id: str, *, confirm: bool, identity: ChannelIdentity
    ) -> str:
