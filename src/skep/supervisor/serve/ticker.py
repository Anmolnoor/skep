...
class Ticker(threading.Thread):
    ...
    def __init__(self, holder: ConfigHolder, store: RunStore, runner: object | None = None) -> None:
    ...
    def _probe_provider_health(self) -> None:
        ...
        for health in run_provider_health_checks(self._store, list_models=_probe_list_models(home)):
            ok = bool(health.reachable and health.model_found)
            if not ok:
                logger.warning("provider %r unhealthy: %s", health.provider_id, health.error)
            self._push_provider_transition(health.provider_id, ok=ok, error=health.error)

    def _push_provider_transition(self, provider_id: str, *, ok: bool, error: str | None) -> None:
