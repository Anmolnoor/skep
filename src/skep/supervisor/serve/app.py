    ...
    *,
        ...
        try:
            ...
            if stale_session:
                ...
                logging.getLogger("skep.serve").info(
                    "cleared %d session-approved shell command(s) from the "
                    "previous serve session",
                    len(stale_session),
                )
            ...
            if dropped_rules:
                logging.getLogger("skep.serve").info(
                    "cleared %d session-approved policy rule(s) from the "
                    "previous serve session",
                    dropped_rules,
                )
        except Exception:
    ...
    def create_repo_branch(name: str, body: BranchRequest) -> dict[str, Any]:
        return create_branch(
            holder, name, name=body.name, from_ref=body.from_ref, store=run_store
        )
    ...
    def preview_workon(body: WorkonRequest) -> dict[str, Any]:
        ...
        return workon_preview(
            holder, run_store, path=body.path, pack=body.pack, phase=body.phase
        )
    ...
    def post_policy_group_fork(name: str, body: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            return set_policy_group(
                ...
                repoint_project=(
                    None
                    if body.get("repoint_project") is None
                    else str(body["repoint_project"])
                ),
            )
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
