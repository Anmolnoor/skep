...
def create_branch(
    ...
    *,
    ...
    if created.returncode != 0:
        raise HTTPException(
            status_code=502, detail=f"git branch failed: {created.stderr.strip()}"
        )
...
def push_branch(
    ...
    if pushed.returncode != 0:
        raise HTTPException(
            status_code=502, detail=f"git push failed: {pushed.stderr.strip()}"
        )
...
def push_baseline(
    ...
    if not name:
        raise HTTPException(
            status_code=400, detail="cannot determine a default branch to push"
        )
    exists = subprocess.run(
        ["git", "-C", str(resolved), "rev-parse", "--verify", "--quiet", f"refs/heads/{name}"],
        capture_output=True,
    ...
    if probe.returncode != 0:
        raise HTTPException(
            status_code=502, detail=f"git ls-remote failed: {probe.stderr.strip()}"
        )
...
def set_policy_group(
    ...
    *,
    ...
    if fork_from not in groups:
        raise ValueError(
            f"no policy group {fork_from!r} to fork; known: {sorted(groups)}"
        )
    if name in groups:
        raise ValueError(
            f"policy group {name!r} already exists — a fork needs a fresh name"
        )
...
def detach_policy_group(store: RunStore, *, project_id: str, name: str) -> dict[str, Any]:
    ...
    return _update_project_group_list(
        store, project_id, [n for n in current if n != name]
    )
    ...
    for ref, label in ((source, "source"), (into, "into")):
        ...
        if probe.returncode != 0:
            raise HTTPException(
                status_code=404,
                detail=(
                    f"unknown {label} ref {ref!r} — if it is new on the remote, "
                    "refresh_repo first"
                ),
            )
    ...
    if record is None:
        ...
        if len(prefix) >= _MIN_TASK_ID_PREFIX:
            ...
            if len(matches) == 1:
                ...
                raise HTTPException(
                    status_code=409,
                    detail=f"task id prefix {task_id!r} is ambiguous: "
                    + ", ".join(matches[:5]),
                )
    if record is None:
...
def apply_patch(
    ...
    *,
    ...
    store.resolve_approval(
        review_id, approved=True, actor=actor, note=note, landing_branch=target
    )
    return target
...
def remember_commands_for_session(
    store: RunStore, commands: list[list[str]]
) -> list[list[str]]:
