...
def queen_filesystem_decision(
    ...
    if not path.strip():
        return AutonomyDecision(
            verdict="deny", reason="filesystem.deny.missing_path", detail=path
        )
    try:
...
def read_file_branch_aware(
    ...
    *,
    ...
    if ref is not None:
        if repo_dir is None:
            return {"error": f"{target} is not inside a git repository, so ref={ref!r} "
                    "cannot be resolved"}
    ...
    hint = (
        f" — landed work lives on {', '.join(skep_branches[:5])}; "
        "pass ref to read from one"
        if skep_branches
        else ""
    )
    return {
        "error": (
            f"not a file on the checked-out branch ({checked_out!r}): {target}{hint}"
        )
    }
...
def search_files_result(
    ...
    *,
    ...
    try:
        proc = subprocess.run(
            argv, capture_output=True, text=True, timeout=SEARCH_TIMEOUT_SECONDS
        )
