...
def add_registry_routes(app: FastAPI, *, holder: ConfigHolder, run_store: RunStore) -> None:
    ...
    def ops_plan(body: OpsRunRequest) -> dict[str, Any]:
        ...
        return {
            "decision": {"verdict": decision.verdict, "reason": decision.reason,
    ...
    def ops_run(body: OpsRunRequest) -> dict[str, Any]:
        ...
        if not decision.allows_execution():
            raise HTTPException(
                status_code=409, detail=f"{decision.verdict}: {decision.reason}"
            )
        if decision.dry_run:
            raise HTTPException(
                status_code=409,
        ...
        try:
            result = execute_ops(
                decision, capability=body.capability, arguments=body.arguments
            )
    ...
    def put_channel(channel: str, body: ChannelConfigRequest) -> dict[str, Any]:
        ...
        if body.notification_level is not None and body.notification_level not in (
            ...
            raise HTTPException(
                status_code=400,
                detail=(
                    f"notification_level must be one of {', '.join(NOTIFICATION_LEVELS)}"
                ),
            )
        ...
        run_store.upsert_channel_config(
            ChannelConfig(
                ...
                auto_thread=(
                    current.auto_thread if body.auto_thread is None else body.auto_thread
                ),
                allowed_users=(
