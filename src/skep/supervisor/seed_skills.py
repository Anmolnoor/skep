...
def sync_skill_shelves(store: RunStore) -> dict[str, dict[str, Any]]:
    ...
    return {
        entry: load_seed_skills(store, root=Path(entry), provenance=EXTERNAL_PROVENANCE)
...
def load_seed_skills(
    ...
    for directory in sorted(shelf.iterdir()):
        ...
        if store.get_template(pack.name) is not None:
            ...
            continue
        template = dataclasses.replace(template_from_skill_md(pack), provenance=provenance)
