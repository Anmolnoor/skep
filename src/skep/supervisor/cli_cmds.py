...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    try:
        outcome = run_task(
            ...
            coding_engine=(resolved_policy.coding_engine if resolved_policy is not None else ""),
        )
    finally:
        if tail is not None:
            tail.stop()
...
def register_supervisor_commands(
    ...
    branch_delete.add_argument("--remote", action="store_true", help="also delete origin/<name>")
    ...
    pr_list.add_argument("--state", default="open", choices=["open", "closed", "merged", "all"])
