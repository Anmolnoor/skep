...
def cmd_hermes_import(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        existing_titles = {chat.title for chat in store.list_chats() if chat.source == CHAT_SOURCE}
        imported_chats = 0
        for title, messages in sessions:
    ...
    if staged:
        ...
        for memory_class in sorted(per_class):
            print(
                f"  skep memory approve-batch --actor {IMPORT_ACTOR} --memory-class {memory_class}"
            )
...
def register_hermes_commands(
    ...
    import_p.add_argument("--hermes-home", default="~/.hermes", help="Hermes state directory")
