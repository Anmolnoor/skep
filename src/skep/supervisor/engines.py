...
CODING_ENGINES: dict[str, CodingEngine] = {
    ...
    "claude_code": CodingEngine(
        ...
        summary=(
            "Claude Code, headless (--print). Confined by the sandbox, "
    ...
    "codex": CodingEngine(
        ...
        summary=(
            "Codex CLI, headless. Confined by the sandbox, not the capability layer."
        ),
    ),
    "aider": CodingEngine(
        name="aider",
        ...
        summary=(
            "Aider, pinned --no-auto-commit. Confined by the sandbox, "
            "not the capability layer."
        ),
    ),
}
...
def resolve_engine(name: str | None) -> CodingEngine:
    ...
    if engine is None:
        raise ValueError(
            f"unknown coding_engine {name!r}; known: {', '.join(engine_names())}"
        )
    return engine
