    ...
    *,
    ...
    if capability in OPS_NETWORK_CAPABILITIES:
        ...
        if not host or host not in allowed_hosts:
            return OpsDecision("deny", "ops.deny.network_probe_host_not_allowlisted", host or None)
        if not network_enforcement_available:
            return OpsDecision("deny", "ops.deny.network_probe_enforcement_unavailable", host)
    ...
    if capability in OPS_MUTATING_CAPABILITIES:
        ...
        if capability == "ops.backup.run":
            ...
            if not source or not dest:
                return OpsDecision("deny", "ops.deny.backup_requires_source_and_dest", capability)
