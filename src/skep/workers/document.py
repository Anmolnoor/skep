...
def compose_messages(
    instructions: str, files: Sequence[tuple[str, str]]
) -> list[dict[str, Any]]:
...
def draft_verification(
    draft: str, must_include: Sequence[str]
...
def _execute(
    ...
    try:
        ...
        result = CodingWorkerResult(
            ...
            commands=[],
            verification=Verification(
                outcome=VerificationOutcome.NOT_ATTEMPTED, details=failure
            ),
            artifacts=[
                Artifact(
                    kind="event_log",
    ...
    artifacts = [
        ...
        Artifact(
            kind="file", path=DRAFT_MD_PATH, sha256=_sha256_file(workspace / DRAFT_MD_PATH)
        ),
    ]
