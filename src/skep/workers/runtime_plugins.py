...
class ResumeCheckpointPlugin:
    ...
    def react_checkpoint_from_state(
        ...
        return ReactCheckpoint(
            ...
            changed_files=tuple(
                item for item in _sequence_or_empty(raw.get("changed_files"))
                if isinstance(item, str)
            ),
            commands=tuple(
                dict(item) for item in _sequence_or_empty(raw.get("commands"))
                if isinstance(item, Mapping)
            ),
            verification=dict(verification) if isinstance(verification, Mapping) else None,
            provider_calls=provider_calls
...
def _flag_variant_of(
    argv: Sequence[str], entries: Sequence[Sequence[str]]
) -> Sequence[str] | None:
        ...
        *,
        ...
        if allowlist_variant is not None:
            return RuntimePolicyDecision(
                ...
                detail=(
                    f"{command} (bare-flag variant of allowlisted "
                    f"'{' '.join(allowlist_variant)}')"
                ),
            )
        return RuntimePolicyDecision(
