...
class ServeClient:
    ...
    def __init__(
        self, base_url: str, token: str, *, client: HttpxClientLike | None = None
    ) -> None:
...
COMMANDS: dict[str, dict[str, str]] = {
    ...
    "resume": {
        ...
        "desc": "continue a crashed/timed-out run from its checkpoint — "
        "model-free (confirm card)",
    },
    "browser": {
        "usage": "/browser",
        "desc": "register the built-in Playwright browser under the browse "
        "scope (confirm card)",
    },
    "status": {
        "usage": "/status",
...
class _TurnPrinter:
    ...
    def feed(self, event: str, data: dict[str, Any]) -> None:
        if event == "message":
            ...
            self._write(
                "    " + _c(DIM, _tool_summary(name, data.get("result"), self.url)) + "\n"
            )
            ...
            if decision:
                ...
                note = f"ran without asking — covered by {covered}" if covered else (
                    "ran without asking — a standing grant covers it"
                )
...
class ChatRepl:
    ...
    def send(self, text: str) -> None:
        try:
            ...
            self.resolve_cards(
                [a for a in detail["actions"] if a.get("status") == "proposed"]
            )
            actions = self._post_message(text)
    ...
    def _apply_card_verb(self, action: dict[str, Any], verb: str) -> None:
        ...
        if action.get("source") == "operator":
            ...
            result = self.client.post(
                f"/api/chats/{chat_id}/commands/{action_id}/{verb}"
            )
    ...
    def _tail_after_action(self, action_id: str, *, chat_id: str | None = None) -> None:
        ...
        row = next(
            (a for a in detail["actions"] if a.get("action_id") == action_id), None
        )
    ...
    def tail_run(self, task_id: str) -> None:
        ...
        print(
            _c(
                DIM,
                f"run {self._task_ref(task_id)} — watching "
    ...
    def _approval_gate(self, task_id: str) -> None:
        ...
        pending = [
            a
            for a in self.client.get("/api/approvals")["approvals"]
    ...
    def run_command(self, text: str) -> None:
        ...
        if name == "phase":
            ...
            self._propose_command(
                "set_project_phase", {"project_id": args[0], "phase": args[1]}
            )
            return
        if name == "land":
            if not args:
        ...
        if name == "model":
            ...
            self._propose_command(
                "set_assistant_model", {"model": args[0], "scope": scope},
