...
def test_missing_path_is_denied(
    store: RunStore, holder: ConfigHolder
) -> None:
...
def test_read_inside_root_executes_in_turn(
...
def test_deny_rule_refuses_without_a_card(
    config: SupervisorConfig, ollama: FakeOllama
) -> None:
    client, chat_id = chat_client(config, ollama)
    store = RunStore(config.db_path)
...
def test_read_file_explicit_ref_reads_landed_content(
...
def test_read_file_miss_teaches_the_landing_branches(
    store: RunStore, tmp_path: Path
) -> None:
...
def test_deny_rule_still_denies_a_ref_read(
    config: SupervisorConfig, ollama: FakeOllama
) -> None:
    ...
    ollama.script_tool_call(
        "read_file", {"path": str(vault / "key.txt"), "ref": "skep/maintain"}
    )
    ollama.script_reply("that path is denied")
    events = sse_events(
        client.post(f"/api/chats/{chat_id}/messages", json={"content": "read the key"}).text
