...
def _heartbeat_run(count: int) -> list[Event]:
    events = [
        _event(1, EventType.TASK_START, _start_payload(), ts="2026-07-15T10:00:00Z")
    ]
...
def test_terminal_failure_lands_one_honest_line_and_pushes_out(
    ...
    try:
        ...
        task = mint_task(
            workspace=tmp_path / "ws", instructions="fix it", budget=DEFAULT_BUDGET
        )
...
def test_status_stream_reports_a_run_that_gated_before_subscribe(
    ...
    try:
        task = mint_task(
            workspace=Path("/tmp/ws-x"), instructions="x", budget=DEFAULT_BUDGET
        )
