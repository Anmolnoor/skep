...
def test_at_least_metric_status() -> None:
    ...
    assert (
        metric("completion", 0.9, threshold=0.95, warn_threshold=0.85).status == "warn"
    )
...
def test_at_most_metric_status() -> None:
    ...
    assert (
        metric(
            "runs_per_task", 3, threshold=2, comparison="at_most", warn_threshold=4
        ).status
        == "warn"
    )
    assert (
        metric("drift", 5, threshold=0, comparison="at_most", warn_threshold=1).status
        == "fail"
    )
