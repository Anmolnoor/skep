...
def test_merge_is_sorted_and_deduped_regardless_of_input_order(tmp_path: Path, repo: Path) -> None:
...
def test_unmanaged_repo_without_roots_still_gets_no_allowlist(tmp_path: Path, repo: Path) -> None:
...
def test_registry_hosts_follow_enforceability_and_strategy(tmp_path: Path, repo: Path) -> None:
