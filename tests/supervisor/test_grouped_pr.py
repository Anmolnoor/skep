...
def test_grouped_pr_skips_patchless_members_with_an_honest_note(
    ...
    store = RunStore(config.db_path)
    try:
        task = mint_task(workspace=two_file_repo, instructions="script only", budget=DEFAULT_BUDGET)
        store.create_run(task, repo=two_file_repo.resolve(), ref=None, execution_mode="sandbox")
