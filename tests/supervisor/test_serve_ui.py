...
def test_home_activity_feed_strip_and_banner() -> None:
    ...
    for selector in (".activity-list", ".activity-dot-failed", ".schedule-strip",
                     ".welcome-back"):
...
def test_topbar_queen_status_claims_only_sourced_fields() -> None:
    ...
    assert ("window ${llm.num_ctx} (${llm.num_ctx_source}) "
            "· tools ${llm.tool_delivery}") in dock
...
def test_projects_cards_and_detail_route() -> None:
    ...
    for call in ('api("GET", "/api/projects")', 'api("GET", "/api/schedules")',
...
def test_assign_guided_flow_and_template_prefill() -> None:
    ...
    assert assign.index('"3. Advanced settings"') \
        < assign.index('el("label", {}, "execution")')
...
def test_notes_render_markdown_and_tasks_group_by_due() -> None:
    ...
    order = ['{ label: "Overdue"', '{ label: "Due today"', '{ label: "Upcoming"',
             '{ label: "No due date"',
    ...
    for selector in (".note-tag", ".chip.tone-bad", ".chip.tone-warn",
                     ".task-title.done", ".task-group"):
        assert selector in css, selector
...
def test_memory_chips_and_chat_sidebar_search_pinning() -> None:
    ...
    for selector in (".chip", ".chip.upper", ".chat-sidebar-search",
                     ".chat-pin.pinned", ".sidebar-chat-pinned-label"):
        assert selector in css, selector
...
def test_chat_composer_matches_cursor_inspired_chrome() -> None:
    ...
    for selector in (".strip-pill", ".strip-project-select", ".strip-btn",
...
def test_v75_foundation_tokens_helpers_and_rail_groups() -> None:
    ...
    for token_name in (
        "--accent-2:", "--shadow-card:", "--shadow-raised:",
        "--transition-fast:", "--transition-normal:", "--radius-xl:",
        "--accent:", "--shadow-pop:", "--shadow-focus:", "--radius-pill:",
    ):
    ...
    for helper in (
        "function relativeTime", "function buildTabBar",
        "function buildFilterBar", "function buildSparkline",
    ):
    ...
    for selector in (
        ".tab-bar", ".tab-button.active", ".tab-content",
        ".filter-bar", ".filter-tab.active", ".filter-count",
        ".stat-sparkline", ".spark-ok", ".spark-bad",
    ):
        assert selector in css, selector
...
def test_runs_page_renders_filter_tabs_and_grouped_cards() -> None:
    ...
    for key in ('key: "all"', 'key: "running"', 'key: "pending"',
                'key: "completed"', 'key: "failed"'):
        assert key in runs_view, key
    # Cards keep search + dimming + verify.
    assert 'class: `run-card searchable${run.state === "superseded" ? " superseded" : ""}`' \
        in runs_view
    ...
    for selector in (".run-card", ".run-card.superseded { opacity: .55; }",
                     ".run-group-count", ".run-card-verify.passed"):
        assert selector in css, selector
...
def test_policies_sections_help_and_shell_editor() -> None:
    ...
    for selector in (".policy-section", ".policy-section-header",
                     ".field-help", ".shell-cmd-row"):
        assert selector in css, selector
...
def test_run_detail_timeline_tabs_and_copy_id() -> None:
    ...
    assert ('const TIMELINE_STATES = ["created", "dispatched", "running", '
            '"pending_approval", "completed"]') in source
    ...
    for tab in ('{ key: "events", label: "Events" }',
    ...
    for selector in (".run-timeline", ".timeline-node.reached .timeline-dot",
                     ".timeline-connector", ".run-id-row"):
        assert selector in css, selector
...
def test_templates_and_skills_split_into_tabs() -> None:
    ...
    for selector in (".template-card", ".template-card-use",
                     ".skill-stepper", ".skill-step.current .skill-step-dot"):
        assert selector in css, selector
...
def test_command_palette_navigates_and_never_mutates() -> None:
    ...
    for ws in ("home", "chat", "runs", "approvals", "assign", "projects",
               "schedules", "templates", "notes", "memory", "policies",
               "setup", "settings"):
    ...
    search = source[source.index("function installSearch") : source.index(
        "// ---------- v75-F8")]
...
def test_pending_cards_reconcile_without_a_reload() -> None:
    ...
    finally_block = source[
        finally_start : source.index("updateContextMeter()", finally_start)
    ]
...
def test_settings_worker_tab_shows_the_roster_above_the_provider_card() -> None:
    ...
    for field in ("c.summary", "c.lands", "c.needs_provider", "c.needs_network",
                  "e.summary", "e.external", "e.binary", "c.detail", "e.detail"):
