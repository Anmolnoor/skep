...
def test_ladder_happy_path_lands_template_and_snapshot(tmp_path: Path) -> None:
    ...
    try:
        record = draft_pack(store, directory, grants=("python scripts/query.py",))
        ...
        promoted, template = promote_pack(store, config, "homelab-control", human_action=True)
        ...
        again, none_template = promote_pack(store, config, "homelab-control", human_action=True)
...
def test_suspend_removes_template_and_rollback_is_terminal(tmp_path: Path) -> None:
    ...
    try:
        ...
        reactivated, template = promote_pack(store, config, "homelab-control", human_action=True)
...
def test_materialize_packs_for_run_copies_only_referenced_active_packs(
    ...
    try:
        ...
        plain.mkdir()
        assert materialize_packs_for_run(store, config, plain, [["uv", "run", "pytest"]]) == []
