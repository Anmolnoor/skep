...
def test_batch_cards_then_confirm_dispatches_all_in_parallel(
    ...
    tasks = [
        ...
        {"repo": str(repo_two), "instructions": "Fix the other bug. MODE:happy",
         "execution_mode": "workspace"},
    ]
    ...
    assert {run["repo"] for run in runs.values()} == {str(repo.resolve()),
...
def test_batch_validation_is_honest(repo: Path, config: SupervisorConfig) -> None:
    ...
    try:
        ...
        with pytest.raises(ValueError, match=f"caps at {BATCH_DISPATCH_CAP}"):
            mutate(
                [{"repo": str(repo), "instructions": "x. MODE:happy"}]
                * (BATCH_DISPATCH_CAP + 1)
            )
...
def test_a_member_engine_reaches_that_member_only(
            ...
            execute_mutation(
                "batch_dispatch",
                {
                    "tasks": [
                        {"repo": str(repo), "instructions": "A. MODE:happy",
                         "execution_mode": "workspace"},
                        {"repo": str(repo), "instructions": "B. MODE:happy",
                         "execution_mode": "workspace", "engine": "builtin"},
                    ]
                },
                store=store,
                holder=holder,
        ...
        with pytest.raises(Exception, match="claude_code"):
            execute_mutation(
                "batch_dispatch",
                {
                    "tasks": [
                        {"repo": str(repo), "instructions": "A. MODE:happy",
                         "execution_mode": "workspace", "engine": "gpt5"},
                    ]
                },
                store=store,
                holder=holder,
...
def test_batch_members_get_separate_audit_trails(
    ...
    ollama.script_tool_call(
        ...
        {
            "tasks": [
                {"repo": str(repo), "instructions": "One. MODE:happy",
                 "execution_mode": "workspace"},
                {"repo": str(repo), "instructions": "Two. MODE:happy",
                 "execution_mode": "workspace"},
            ]
        },
    )
