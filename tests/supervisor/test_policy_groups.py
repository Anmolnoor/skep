...
def test_groups_round_trip_and_builtins_resolve_without_a_row(
    ...
    try:
        ...
        save_policy_group(
            store, "npm-api", {"default_network": ["api.npmjs.example"]}
        )
        groups = stored_policy_groups(store)
...
def test_dangling_group_fails_the_dispatch_closed(
    config: SupervisorConfig, repo: Path
) -> None:
    store = RunStore(config.db_path)
    try:
...
def test_group_verbs_ride_the_mutation_path(
    config: SupervisorConfig, repo: Path
) -> None:
    ...
    try:
        ...
        attached = _mutate(
            store, config, "attach_policy_group",
        ...
        detached = _mutate(
            store, config, "detach_policy_group",
            {"project_id": "grouped", "name": "npm-api"},
        )
    ...
    try:
        ...
        with pytest.raises(ValueError, match="fresh name"):
            set_policy_group_action(
                store, name="npm-api", policy={}, fork_from="npm-api-2"
            )
...
def test_group_http_routes_are_operator_direct(
    config: SupervisorConfig, repo: Path
) -> None:
...
def test_group_verbs_pass_the_operator_command_gate(
    config: SupervisorConfig, repo: Path
) -> None:
