...
def _seed_http_server(config: SupervisorConfig) -> None:
    ...
    try:
        save_mcp_server(
            store,
            MCPServerConfig(
                server_id="remote", transport="http", url="https://mcp.example/mcp"
            ),
        )
    finally:
        store.close()
...
def test_read_shaped_tool_runs_inside_the_turn(
    ...
    ollama.script_tool_call(
        "call_mcp_tool", {"server_id": "issues", "tool": "list_issues"}
    )
...
def test_unknown_tool_cards_then_executes_on_confirm(
    ...
    client, chat_id = chat_client(config, ollama)
    ollama.script_tool_call(
        "call_mcp_tool", {"server_id": "issues", "tool": "frobnicate_gizmo"}
    )
            ...
            "scopes": [
                {
                    "scope": "mcp",
                    "deny": [
                        {"rule_id": "no-gizmos", "action": "call", "pattern": "issues:frob*"}
                    ],
                }
            ],
        },
...
def test_allow_always_writes_a_learned_rule_that_survives_restart(
    ...
    ollama.script_tool_call(
        "allow_mcp_tool", {"server_id": "issues", "tool": "frobnicate_gizmo"}
    )
...
def test_http_transport_is_transparent_to_the_chat_surface(
    ...
    ollama.script_tool_call(
        "call_mcp_tool", {"server_id": "remote", "tool": "frobnicate_gizmo"}
    )
