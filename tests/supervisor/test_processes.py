...
def test_a_run_grant_never_covers_a_daemon(config: SupervisorConfig) -> None:
    ...
    try:
        ...
        assert queen_shell_decision(store, holder, command="sleep 5", cwd=None) is not None
        assert (
            mutation_execution_decision(
...
def test_unknown_proc_id_error_teaches(config: SupervisorConfig) -> None:
    ...
    try:
        with pytest.raises(ValueError, match="known:"):
            execute_read_tool("read_process_log", {"proc_id": "nope"}, store=store, holder=holder)
    finally:
        store.close()
