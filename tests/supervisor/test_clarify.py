...
def test_the_next_message_is_the_answer(config: SupervisorConfig, ollama: FakeOllama) -> None:
    client, chat_id = chat_client(config, ollama)
...
def test_empty_question_degrades_to_a_tool_error(
    ...
    events = sse_events(client.post(f"/api/chats/{chat_id}/messages", json={"content": "hm"}).text)
