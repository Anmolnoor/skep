...
def test_provider_drop_mid_turn_persists_an_honest_line(
    ...
    assert any(name is None and "provider dropped" in d["content"] for name, d in events)
...
def test_malformed_tool_args_feed_back_and_the_turn_continues(
    ...
    assert events[-1] == ("done", {"state": "complete"})
    streamed = "".join(d.get("content", "") for name, d in events if name is None)
...
def test_read_only_turn_never_cards_and_answers_the_mutation_attempt(
    ...
    offered = {t["function"]["name"] for t in ollama.chat_bodies()[0].get("tools", [])}
    from skep.supervisor.serve.tools import MUTATING_TOOL_NAMES

    assert offered and not (offered & MUTATING_TOOL_NAMES)


def test_btw_runs_beside_a_pending_card(config: SupervisorConfig, ollama: FakeOllama) -> None:
    ...
    streamed = "".join(d.get("content", "") for name, d in sse_events(beside.text) if name is None)
...
def test_setup_project_takes_the_engine_first_class(repo: Path, config: SupervisorConfig) -> None:
...
def test_an_explicit_override_beats_the_engine_sugar(repo: Path, config: SupervisorConfig) -> None:
...
def test_repo_state_tool_shows_branches_and_default(repo: Path, tmp_path: Path) -> None:
...
def test_get_run_guides_landing_for_completed_unlanded_patch(repo: Path, tmp_path: Path) -> None:
...
def test_land_run_tool_refuses_when_nothing_to_land(repo: Path, config: SupervisorConfig) -> None:
...
def test_bind_chat_project_remembers_where_the_chat_works(tmp_path: Path) -> None:
    ...
    try:
        ...
        _bind_chat_project(store, None, chat.chat_id, "workon", {}, {"project_id": "proj-a"})
...
def test_list_approvals_carries_resolved_tail_and_resume_chain(repo: Path, tmp_path: Path) -> None:
...
def test_distinct_reads_are_never_refused(config: SupervisorConfig, ollama: FakeOllama) -> None:
...
def test_seeded_repeat_gets_one_nudge_then_refusal_in_the_new_turn(
    ...
    ollama.script_tool_call("list_runs", {"limit": 1})
    ollama.script_reply("nothing running")
    sse_events(client.post(f"/api/chats/{chat_id}/messages", json={"content": "anything?"}).text)
    ...
    ollama.script_reply("still nothing — want me to dispatch something?")
    second = sse_events(client.post(f"/api/chats/{chat_id}/messages", json={"content": "??"}).text)
...
def test_turn_status_events_name_the_wait(config: SupervisorConfig, ollama: FakeOllama) -> None:
