...
def test_push_fails_soft_at_every_gate(config: SupervisorConfig) -> None:
    ...
    try:
        ...
        assert push_to_chat_channel(store, config.home, chat_id, "x", send_discord=_boom) is False
...
def test_discord_push_with_run_ref_sends_content_plus_embed(
    ...
    try:
        ...
        assert push_to_chat_channel(store, config.home, chat_id, "tick digest", send_discord=_send)
