...
class _DiscordHarness:
    ...
    def _register(
        self, token: str, application_id: str, commands: list[dict[str, object]]
    ) -> int:
        self.registrations.append((token, application_id, commands))
        return self.register_status

    def _create_thread(
        self, token: str, channel_id: str, message_id: str, name: str
    ) -> str | None:
...
def test_discord_identifies_and_routes_a_message_through_a_real_turn(
    ...
    try:
        ...
        try:
            ...
            assert harness.sent == [
                ("discord-bot-token", "42", {"content": "hello from the hive"})
            ]
...
def test_discord_reaction_confirms_the_waiting_card(
    repo: object, config: SupervisorConfig
) -> None:
    from skep.supervisor import RunStore
...
def test_discord_mention_spawns_a_thread_and_binds_the_conversation_there(
    ...
    try:
        ...
        try:
            ...
            assert harness.sent == [
                ("discord-bot-token", "t-900", {"content": "threaded hello"})
            ]
...
def test_discord_thread_creation_failure_falls_back_in_channel(
    ...
    try:
        ...
        try:
            ...
            assert harness.gateway.session_once() is True
            assert harness.sent == [
                ("discord-bot-token", "42", {"content": "in-channel fallback"})
            ]
        finally:
            harness.close()
    finally:
...
def test_discord_image_attachment_rides_the_user_message(
    ...
    try:
        ...
        try:
            ...
            stored = (
                config.home / "chat-attachments" / binding.chat_id / user_row.attachments[0]
            )
