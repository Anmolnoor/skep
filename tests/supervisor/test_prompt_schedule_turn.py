...
def test_prompt_turn_refuses_web_reads_unattended(
    ...
    try:
        ...
        assert ok is True
        tool_rows = [m.content for m in store.chat_messages(chat.chat_id) if m.role == "tool"]
...
def test_prompt_turn_without_a_living_chat_or_config_fails_honestly(
    ...
    try:
        reply, ok = run_scheduled_prompt(store, holder, runner, config.home, _schedule(None), None)
