...
def test_personal_dev_character_worktree_free_registries_only() -> None:
    ...
    assert (
        decide(resolved, "network", "connect", "example.com", template="personal-dev").verdict
        == "require_approval"
    )
    assert (
        decide(resolved, "shell", "run", "cargo build", template="personal-dev").verdict
        == "require_approval"
    )
...
def test_homelab_ops_character_declared_paths_mcp_gated() -> None:
    ...
    assert (
        decide(resolved, "filesystem", "write", "/etc/passwd", template="homelab-ops").verdict
        == "require_approval"
    )
    assert (
        decide(resolved, "shell", "run", "systemctl status nginx", template="homelab-ops").verdict
        == "allow"
    )
    assert (
        decide(resolved, "mcp", "call", "anything:at_all", template="homelab-ops").verdict
        == "require_approval"
    )
    # v41-F3: email mirrors the mcp-gated stance — nothing flows unasked.
    assert (
        decide(resolved, "email", "read", "mail:read_inbox", template="homelab-ops").verdict
        == "require_approval"
    )
    assert (
        decide(resolved, "email", "send", "mail:send_message", template="homelab-ops").verdict
        == "require_approval"
    )
...
def test_assistant_character_reads_free_mutations_carded() -> None:
    ...
    assert (
        decide(resolved, "filesystem", "write", "/notes.md", template="assistant").verdict
        == "require_approval"
    )
