...
def test_allowlisted_update_normalizes() -> None:
    result = normalize_inbound(
        {"message": {"chat": {"id": 42}, "text": "audit my repo"}}, _CONFIG
    )
...
def test_callback_gate_requires_both_chat_and_presser_allowlisted() -> None:
    ...
    allowed = handle_callback(
        click="confirm", action_class="dispatch_run",
        chat_id="42", from_id="42", config=_CONFIRM_CONFIG,
    )
    assert allowed.allowed is True

    bystander = handle_callback(
        click="confirm", action_class="dispatch_run",
        chat_id="42", from_id="666", config=_CONFIRM_CONFIG,
    )
    ...
    shell = handle_callback(
        click="confirm", action_class="allow_command_review",
        chat_id="42", from_id="42", config=_CONFIRM_CONFIG,
    )
    ...
    unknown = handle_callback(
        click="explode", action_class="dispatch_run",
        chat_id="42", from_id="42", config=_CONFIRM_CONFIG,
    )
