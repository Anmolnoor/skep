...
def test_upload_serve_and_message_roundtrip(config: SupervisorConfig, ollama: FakeOllama) -> None:
    ...
    assert (
        client.post(f"/api/chats/{chat_id}/attachments", content=b"not an image").status_code == 400
    )
...
def test_model_sees_placeholder_without_vision_and_images_with_it(
    ...
    client.post(f"/api/chats/{chat_id}/messages", json={"content": "look", "attachments": [name]})
