...
def test_setup_prints_the_next_steps(
...
def test_setup_wizard_silent_without_tty(
...
def test_provider_list_and_health_cli(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    from skep.supervisor.cli_cmds import build_config
...
def test_project_setup_writes_the_slug_binding_for_a_managed_clone(
    ...
    code = _run_cli(home, "project", "setup", str(managed), "--pack", "public_free",
                    "--project-id", "widget")
    out = capsys.readouterr().out
    assert code == 0, out
...
def test_project_setup_invents_no_slug_for_a_directory_outside_the_root(
    ...
    home = tmp_path / "home"
    code = _run_cli(home, "project", "setup", str(repo), "--pack", "public_free",
                    "--project-id", "workon-proj")
    out = capsys.readouterr().out
    assert code == 0, out
...
def test_project_preview_reports_the_bindings_setup_will_write(
    ...
    assert _run_cli(home, "project", "preview", str(managed), "--pack", "public_free",
                        "--project-id", "pv") == 0
    preview_out = capsys.readouterr().out
    assert _run_cli(home, "project", "setup", str(managed), "--pack", "public_free",
                        "--project-id", "pv") == 0
    setup_out = capsys.readouterr().out
...
def test_the_v100_sequence_ends_with_both_bindings(
    ...
    assert _run_cli(
        home, "project", "setup", str(managed), "--pack", "public_free",
        "--project-id", "bench", "--phase", "maintain"
    ) == 0
...
def test_skep_branch_consolidates_task_branches(
    ...
    assert _run_cli(
        home, "branch", "merge", "fixture", "--source", "skep/task-a", "--into", "skep/integration"
    ) == 0
    assert _run_cli(
        home, "branch", "merge", "fixture", "--source", "skep/task-b", "--into", "skep/integration"
    ) == 0
    capsys.readouterr()
...
def test_skep_branch_reports_the_actions_own_refusal(
    ...
    assert _run_cli(
        home, "branch", "merge", "fixture", "--source", "skep/task-a", "--into", default
    ) != 0
