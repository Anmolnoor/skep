...
def test_worker_protocol_resolves_from_the_project_overlay(
...
def test_worker_protocol_garbage_fails_closed_with_the_teach(
    tmp_path: Path, repo: Path
) -> None:
    config = build_config(tmp_path / "home", None)
    store = RunStore(config.db_path)
...
def test_react_knob_rides_the_serve_dispatch_into_the_envelope(
    ...
    envelope = json.loads(
        (config.audit_dir / task_id / "task.json").read_text(encoding="utf-8")
    )
