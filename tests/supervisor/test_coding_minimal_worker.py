...
def test_default_coding_worker_recovers_from_a_denied_command_once(
    ...
    try:
        ...
        outcome = run_task(
            repo, "Create generated.py.", config=config, permissions=permissions
        )
    finally:
        server.stop()
...
def test_react_reply_parses_action_and_done_shapes() -> None:
    ...
    coerced = _parse_react_reply(
        {"done": {"summary": "s", "verify": {"argv": "python3 check.py"}}}
    )
...
def test_react_hollow_trace_fails_honestly(
    ...
    try:
        ...
        server.script_reply(
            _react_reply(
                {"action": {"tool": "filesystem.read", "args": {"path": "hello.py"}}}
            )
        )
...
def test_edit_plan_missing_verify_argv_gets_default_listing() -> None:
    ...
    with pytest.raises(LlmPlanError, match=r"verify\.argv must be a non-empty list"):
        plan_from_payload(
            {"type": "llm_edit_plan", "summary": "s", "files": [], "verify": {}}
        )
...
def test_recovery_context_names_the_sandbox_on_permission_failures() -> None:
    ...
    plan = LlmEditPlan(
        summary="s", files=(), verification=PlannedVerification(argv=("true",))
    )
...
def test_llm_worker_without_counts_stays_none_never_zero_guess(
    repo: Path, tmp_path: Path
) -> None:
