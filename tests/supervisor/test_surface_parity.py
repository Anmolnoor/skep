...
CHAT_ONLY: dict[str, str] = {
    ...
    "set_persona": "profile identity for chats; the operator's face is the deck's `/persona`",
    "set_personality": "THIS chat's reply style; the operator's face is the deck's `/personality`",
...
def test_the_git_family_all_have_operator_faces() -> None:
    ...
    assert not (GIT_FAMILY & _missing()), "the git family lost an operator face — see ADR 0050"
