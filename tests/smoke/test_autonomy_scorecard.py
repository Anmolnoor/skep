...
def test_scenario_1_dispatch_execute_verify_reverify_land(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert any(
            a.status == "approved" and (a.resolved_by or "").startswith("auto:") for a in approvals
        ), approvals
...
def _bind_audit_project(store: RunStore, repo: Path, *, project_id: str, phase: str) -> None:
